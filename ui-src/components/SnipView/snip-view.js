...
import {changeCurrentSnipIndex, saveSnipEdit, snipActions, snipCommentChanged, snipSnipChanged} from '../../store/snipview/snip.Actions';
...
export class SnipView implements OnDestroy {
  currentSnips = [{}];
  currentSnip = {
    snip: '',
    comment: ''
  ...
  getSnipColor = index => {
    return index === this.currentSnipIndex
      ? '#cfc'
      : '#9b9';
  }
  snipClick = index => {
  ...
  onSnipCopy = () => {
    let copyText = this.currentSnip.snip;
