...
function _setCurrentSnips(selectedId, _snipData) {
  const snipsRecord = lodash.find(_snipData.allSnips, { nodeid: selectedId });
...
function _newTreeNode(newNodeID, _snipData) {
  const newSnips = { nodeid: newNodeID, snips: [] };
...
export default function handleActions(state = initialSnipState, action) {
  ...
  switch (action.type) {
    ...
    case 'SelectTreeNode':
      return _setCurrentSnips(action.selectedId, _snipData);
    case 'SelectSnipItem':
      return _selectSnipItem(action.item, _snipData);
    case 'SnipCommentChanged':
      return _snipCommentChanged(action.comment, _snipData);
    case 'SnipSnipChanged':
      return _snipSnipChanged(action.snip, _snipData);
    case 'NewTreeNode':
